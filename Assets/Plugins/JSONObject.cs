/*
 * Comment out when you no longer need to read JSON to disable pretty Print system-wide
 */
#define PRETTY

/*
 * Using doubles will cause errors in VectorTemplates.cs; Unity speaks floats
 * Use floats for numbers instead of doubles (enable if you're getting too many significant digits in string output)
 */
//#define USEFLOAT

/*
 * Currently using a build setting for this one (also it's experimental)
 */
//#define POOLING
...
namespace WebEngageBridge
{
    public class JSONObject : IEnumerable
    {
        ...
        private JSONObject[] GetStringJSONObjects(List<string> objList)
        {
            JSONObject[] jsonArr = new JSONObject[((List<string>)objList).Count];
            for (int index = 0; index < ((List<string>)objList).Count; index++)
            {
                jsonArr[index] = CreateStringObject(((List<string>)objList)[index]);
            }
            return jsonArr;
        }

        private JSONObject[] GetJSONObjects(List<object> objList)
        {
            JSONObject[] jsonArr = new JSONObject[((List<object>)objList).Count];
            for (int index = 0; index < ((List<object>)objList).Count; index++)
            {
                jsonArr[index] = GetJSONObject(((List<object>)objList)[index]);
            }
            return jsonArr;
        }
        ...
        private JSONObject GetJSONObject(object value)
        {
            ...
            if (value is string)
            {
                return CreateStringObject((string)value);
            }
            else if (value is bool)
            {
                return new JSONObject((bool)value);
            }
            else if (value is int)
            {
                return new JSONObject((int)value);
            }
            else if (value is long)
            {
                return new JSONObject((long)value);
            }
            ...
            {
                return new JSONObject((double) value);
            }
            else if (value is DateTime)
            {
                return CreateDateObject((DateTime) value);
            }
            else if (value is Dictionary<string, string>)
            {
                return new JSONObject((Dictionary<string, string>)value);
            }
            else if (value is Dictionary<string, object>)
            {
                return new JSONObject((Dictionary<string, object>)value);
            }
            else if (value is List<string>)
            {
                JSONObject[] jsonArr = GetStringJSONObjects((List<string>)value);
                return new JSONObject(jsonArr);
            }
            else if (value is List<object>)
            {
                JSONObject[] jsonArr = GetJSONObjects((List<object>)value);
                return new JSONObject(jsonArr);
            }
            else
            {
                return CreateStringObject(value.ToString());
            }
        }
        ...
        public JSONObject(Dictionary<string, object> dic)
        {
            ...
            {
                keys.Add(kvp.Key);
                list.Add(GetJSONObject(kvp.Value));
            }
        }
        ...
        {
            type = Type.ARRAY;
            JSONObject[] jsonArr = GetJSONObjects((List<object>)objList);
        ...
        public static JSONObject CreateDateObject(DateTime date)
        {
            return CreateStringObject(((DateTime)date).ToString(DATE_FORMAT));
        }
